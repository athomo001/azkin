...
| `AZKIN_APP_URL` | URL pública del frontend, usada para armar el link/botón del correo de "recuperar contraseña". Opcional: si `AZKIN_CORS_ORIGIN` ya es un dominio real (no `*`), se deriva sola de ahí — solo hace falta fijarla si querés una URL distinta. Si ninguna de las dos está configurada como dominio real, ese correo cae a un modo degradado: solo el token en texto plano, sin link clicable (con warning visible en el log de arranque). | vacío — se deriva de `AZKIN_CORS_ORIGIN` |
