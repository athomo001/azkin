...
export class BackupsPanelComponent {
  ...
  downloadCsvTemplate(): void {
    const csv = [
      ...
      '# http = HTTP / HTTPS | ping = Ping (ICMP) | port = Puerto TCP (generico) | dns = DNS Resolution',
      '# snmp = SNMP Agent | push = Push (Pasivo)',
      '# target es obligatorio salvo si type=push | port es obligatorio si type=port',
      '# el CSV siempre crea monitores de puerto en TCP - para UDP, editalos despues en la UI',
