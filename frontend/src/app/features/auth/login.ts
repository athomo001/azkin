            ...
            <div>
              ...
              <div class="relative">
                <input [type]="showPassword() ? 'text' : 'password'" name="password" [(ngModel)]="password" required
                  class="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-2.5 pr-11 text-white placeholder-zinc-600 focus:outline-none focus:border-emerald-500 transition-colors">
                <button type="button" (click)="showPassword.set(!showPassword())"
                  class="absolute inset-y-0 right-0 flex items-center px-3 text-zinc-500 hover:text-zinc-300 transition-colors"
                  [attr.aria-label]="showPassword() ? lang.t('auth.login.hidePassword') : lang.t('auth.login.showPassword')"
                  [title]="showPassword() ? lang.t('auth.login.hidePassword') : lang.t('auth.login.showPassword')">
                  @if (showPassword()) {
