...
import { DeleteReportDefinitionUseCase } from "./delete-report-definition.usecase";
...
test("DeleteReportDefinitionUseCase elimina una definicion existente", async () => {
  let deletedId: string | null = null;
  const repo = makeRepo({
    findById: async () => makeDefinition(),
    delete: async (id) => {
      deletedId = id;
      return true;
    },
  ...
  const useCase = new DeleteReportDefinitionUseCase(repo, auditLog);
  await useCase.execute("admin-1", "r1");

  assert.equal(deletedId, "r1");
});

test("DeleteReportDefinitionUseCase lanza NotFoundError si no existe", async () => {
