...
function makeFakes(opts: { activeCount?: number; remoteCertPem?: string } = {}) {
  ...
  const client: IFederationClient = {
    requestEnrollment: async (input) => {
      ...
      return { ownCertPem: opts.remoteCertPem ?? VALID_CERT_PEM };
    },
  };
...
test("JoinFederationUseCase rechaza un código con formato inválido", async () => {
  ...
  const useCase = new JoinFederationUseCase(federatedInstances, identity, client, auditLog);
...
test("JoinFederationUseCase decodifica el código, llama a la instancia remota y persiste el registro simétrico", async () => {
  const { federatedInstances, identity, client, auditLog, created, clientCalls } = makeFakes();
  const useCase = new JoinFederationUseCase(federatedInstances, identity, client, auditLog);
